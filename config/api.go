...
func Api() {
	...
	url := "https://parisdata.opendatasoft.com/api/explore/v2.1/catalog/datasets/que-faire-a-paris-/records?select=id%2C%20url%2C%20title%2C%20description%2C%20date_start%2C%20date_end"
	...
	for _, event := range response.Results {
		var creatorID int = 1
		_, err := db.Exec("INSERT IGNORE INTO event (id, title, description, date_start, date_end, url, creatorID) VALUES (?, ?, ?, ?, ?, ?, ?)",
			event.Id, event.Title, event.Description, event.Date_start, event.Date_end, event.Url, creatorID)
		if err != nil {
